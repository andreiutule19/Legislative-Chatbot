...
\begin{tikzpicture}[
  ...
  cloud/.style={rectangle, draw, dashed, rounded corners, minimum width=3cm, minimum height=1.2cm, align=center, font=\small},
  arrow/.style={-{Stealth[length=2mm]}, thick}
]

\node[box] (ui) {React UI\\Sidebar + ChatArea};
\node[box, right=3.8cm of ui] (api) {FastAPI\\Chat routes + health};
\node[box, below=of api] (redis) {Redis\\conversations,\\messages, summaries};
\node[cloud, below=of ui, xshift=1.9cm] (gemini) {Google Gemini\\API key};
\node[cloud, right=of gemini] (vertex) {Vertex AI\\RAG retrieveContexts\\(service account)};

\draw[arrow] (ui) -- node[above, font=\scriptsize] {HTTP / SSE} (api);
\draw[arrow] (api) -- (redis);
\draw[arrow] (api.south west) |- +(-0.3,-0.6) -| (gemini.north);
\draw[arrow] (api.south east) |- +(0.3,-0.6) -| (vertex.north);
...
\begin{longtable}{@{}p{4.2cm}p{9cm}@{}}
\toprule
\textbf{Variable} & \textbf{Description} \\
\midrule
\endhead
\texttt{REDIS\_URL} & Redis URL (container: \texttt{redis://localhost:6379/0}). \\
\texttt{GCP\_PROJECT\_ID} & Google Cloud project id. \\
\texttt{GCP\_LOCATION} & Vertex region (e.g.\ \texttt{europe-west3}). \\
\texttt{RAG\_CORPUS\_NAME} & Full resource name of the RAG corpus. \\
\texttt{GEMINI\_MODEL\_NAME} & Gemini model id. \\
\texttt{GOOGLE\_API\_KEY} & API key for Gemini (Generative Language API). \\
\texttt{GOOGLE\_APPLICATION\_CREDENTIALS} & Filesystem path to service account JSON inside the container. \\
\texttt{CORS\_ORIGINS} & Allowed origins (comma-separated) or \texttt{*}. \\
\texttt{STATIC\_DIR} & Path to built SPA (\texttt{/app/static} in Docker). \\
\texttt{PORT} & Host port for Compose (optional, default 8000). \\
\texttt{GCP\_SA\_KEY\_PATH} & Host path to key file mounted at \texttt{/app/credentials/sa-key.json}. \\
\bottomrule
\end{longtable}
